...
def create_schema():
    ...
    and 5 edge labels, as specified in the request.
    It now also includes a 'properties' column and a GIN index for edge tables.
    """
    conn = None
    try:
        ...
        for label in VERTEX_LABELS:
            ...
            # In a real application, these ranges would be based on your data distribution
            cur.execute(f"""
                CREATE TABLE {GRAPH_NAME}."{label}_part_1_1000" PARTITION OF {GRAPH_NAME}."{label}_parent"
                FOR VALUES FROM ('1'::agtype) TO ('1000'::agtype);
            """)
            cur.execute(f"""
                CREATE TABLE {GRAPH_NAME}."{label}_part_1001_2000" PARTITION OF {GRAPH_NAME}."{label}_parent"
        ...
        for label in EDGE_LABELS:
            ...
            cur.execute(f"""
                CREATE TABLE {GRAPH_NAME}."{label}" (
                    ...
                    start_id AGTYPE,
                    end_id AGTYPE,
                    properties AGTYPE  -- Added properties column for edges
                );
            """)
        ...
        print("- Edge tables now include a 'properties' column and a GIN index on it.")
